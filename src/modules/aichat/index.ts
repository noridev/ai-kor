...
export default class extends Module {
	...
	private async genTextByGemini(aiChat: AiChat, files:base64File[]) {
		...
		// 設定のプロンプトに加え、現在時刻を渡す
		let systemInstructionText = aiChat.prompt + '또한, 현재 날짜와 시간은' + now + '이며, 이는 답변에 참고하고, 시각을 묻기 전까지는 시각 정보를 제공하지 않을 것(또한, 다른 날짜와 시간은 무효로 할 것).';
		// 名前を伝えておく
		if (aiChat.friendName != undefined) {
			systemInstructionText += '또한, 대화 상대의 이름은' + aiChat.friendName + '라고 한다.';
		}
		// ランダムトーク機能(利用者が意図(メンション)せず発動)の場合、ちょっとだけ配慮しておく
		if (!aiChat.fromMention) {
			systemInstructionText += '이러한 메시지는 당신에 대한 메시지가 아님을 유의하고, 답변할 것(대화 상대는 갑자기 말을 건 인식하고 있다).';
		}
		// グラウンディングについてもsystemInstructionTextに追記(こうしないとあまり使わないので)
		if (aiChat.grounding) {
			systemInstructionText += '답변 규칙 2:Google search with grounding.';
		}
		// URLから情報を取得
			...
			if (urlarray.length > 0) {
				for (const url of urlarray) {
					...
					try{
						...
						systemInstructionText += '제공된 추가 URL은 유효하지 않았습니다:URL=>' + url[0]
					...
					if (urlpreview.title) {
						systemInstructionText +=
							'보충으로 제공된 URL은 다음과 같습니다:URL=>' + urlpreview.url
							+'사이트 이름('+urlpreview.sitename+')、';
						if (!urlpreview.sensitive) {
							systemInstructionText +=
							'タイトル('+urlpreview.title+')、'
							+ '설명('+urlpreview.description+')、'
							+ '질문에 있는 URL과 사이트 이름·제목·설명을 조합하여 답변의 참고로 삼을 것.'
							;
							...
							systemInstructionText +=
							'이는 민감한 URL일 가능성이 있기 때문에, 질문에 있는 URL과 사이트 이름만으로 답변에 참고할 것(사용하지 않아도 됨).'
							;
						}
					} else {
						// 多分ここにはこないが念のため
					...
					if (res_data.candidates[0].hasOwnProperty('groundingMetadata')) {
							...
							for (let i = 0; i < checkMaxLength; i++) {
								if (res_data.candidates[0].groundingMetadata.groundingChunks[i].hasOwnProperty('web')) {
									if (res_data.candidates[0].groundingMetadata.groundingChunks[i].web.hasOwnProperty('uri')
										...
										groundingMetadata += `참고(${i+1}): [${res_data.candidates[0].groundingMetadata.groundingChunks[i].web.title}](${res_data.candidates[0].groundingMetadata.groundingChunks[i].web.uri})\n`;
									}
								}
							}
						}
						...
						if (res_data.candidates[0].groundingMetadata.hasOwnProperty('webSearchQueries')) {
							if (res_data.candidates[0].groundingMetadata.webSearchQueries.length > 0) {
								groundingMetadata += '검색어: ' + res_data.candidates[0].groundingMetadata.webSearchQueries.join(',') + '\n';
							}
						}
					}
			...
			for (let i = 0; i < noteData.files.length; i++) {
				if (noteData.files[i].hasOwnProperty('type')) {
					...
					if (noteData.files[i].hasOwnProperty('name')) {
						// 拡張子で挙動を変えようと思ったが、text/plainしかCherryPickで変になってGemini対応してるものがない？
